...
#'
#' @param prior The prior probability that the event was generated by a normal distribution
#' @param z The Z-score of the event, i.e., the number of standard deviations away from the mean
...
p_normal_given_event <- function(prior = 0.5, z = 10, t_df = 2) {
  p_event_given_norm <- pnorm(sd_event, lower.tail = FALSE)
  p_event_given_t <- pt(sd_event, t_df, lower.tail = FALSE)
  ...
  # if denominator is negative then a floating point error has occurred, probably in 1 - prior
  # e.g., 1 - 0.99999999999999999 = -2.220446e-16
  if (denominator < 0) {
    stop("A floating point error has occured. This is usually based on a prior being extremely close to 1.")
  }
