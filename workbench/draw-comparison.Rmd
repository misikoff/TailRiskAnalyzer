...
#' ))
#'
create_side_by_side_plot2 <- function(df, use_log = TRUE, quantiles = c()) {
  # handle use_log
  ...
  time_series_plot <- df |>
    ...
    ggplot2::geom_line(aes(alpha = .data$opacity)) +
    ggplot2::theme(
...
for (i in 1:num_lines) {
  cur_run <- coin_toss_seq(
    100,
    starting_capital = 1,
    min_bet = 0,
    heads_factor = 1.7,
    tails_factor = 0.6,
    betting_fraction = 0.53
  )
  # print(cur_run$results)
...
create_side_by_side_plot2(df,
  use_log = TRUE,
  quantiles = c(0.05, 0.5, 0.95)
)

get_kelly_bet(0.5, 0.4, 0.7)
```